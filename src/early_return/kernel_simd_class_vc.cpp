...
{
	Vc::double_m lanes_active { m }; // set to 0 if lane returns, needs to be used in all assignments.
	// if (x_1 < 0.0)
	...
	y(lanes_active) = SIMD_CLASS_VC_FUNC_1(0.1 + (x_1 + x_2) * (x_1 + x_2)); // perform remaining statement on remaining lanes.
}
